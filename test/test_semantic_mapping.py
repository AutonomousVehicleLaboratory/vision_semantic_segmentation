...
class Test:
	...
	def full_test(self, dir_path="./global_maps", visualize=False):
		...
		for path in path_lists:
			...
			gmap = self.ground_truth_mask[shift_w:generate_map.shape[0]+shift_w, shift_h:generate_map.shape[1]+shift_h]
	...
	def iou(self, gmap, generate_map):
		...
		print("IOU for {}: {}\t{}: {}\tmIOU: {}".format(self.d[0], iou_lists[0], self.d[1], iou_lists[1],
														np.mean(iou_lists)))
		print("Accuracy for {}: {}\t{}: {}\tmean Accuracy: {}".format(self.d[0], acc_lists[0], self.d[1], acc_lists[1],accuracy))
